...
export default function RecurringPaymentForm({
  ...
  const fetchSchedules = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/payments/schedule?userAddress=${address}`);
  ...
  const formatAmount = (amountWei: string) => {
    // Convert from wei back to BTC for display
            ...
            {amount && (
              <div className="bg-purple-500/20 border border-purple-500/30 rounded-lg p-4">
                ...
                <p className="text-purple-100 text-sm">
                  {formatAmount((parseFloat(amount) * 1e18).toString())} BTC {getFrequencyLabel(frequency).toLowerCase()}
                  {maxPayments && ` for ${maxPayments} payments`}
                </p>
                <p className="text-purple-200 text-xs mt-1">
                  Next payment: {new Date(Date.now() + (frequency === "DAILY" ? 86400000 : frequency === "WEEKLY" ? 604800000 : 2592000000)).toLocaleDateString()}
                </p>
              </div>
            )}
          ...
          <div className="space-y-4">
            {schedules.length === 0 ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No Schedules</h3>
                <p className="text-gray-300">You haven&apos;t set up any recurring payments yet.</p>
              </div>
            ) : (
              schedules.map((schedule) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-gray-300 text-sm">
                        {formatAmount(schedule.amount)} BTC {getFrequencyLabel(schedule.frequency).toLowerCase()}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                    <div>
                      ...
                      <p className="text-white">
                        {new Date(schedule.nextPaymentDate).toLocaleDateString()}
                      </p>
                    </div>
                    <div>
                  ...
                  {schedule.payments.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-gray-300 text-sm font-medium mb-2">Recent Payments</h4>
                        ...
                        {schedule.payments.slice(0, 3).map((payment) => (
                          <div key={payment.id} className="flex justify-between items-center text-xs">
                            ...
                            <span className="text-gray-400">
                              {new Date(payment.scheduledFor).toLocaleDateString()}
                            </span>
                          </div>
                        ))}
                      </div>
